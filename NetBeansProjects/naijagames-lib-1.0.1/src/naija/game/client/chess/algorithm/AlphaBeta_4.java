...
public class AlphaBeta_4 {
   ...
   //To be remove - used for testing
    public static void main (String args[]) {
        AlphaBeta_4 ab=new AlphaBeta_4();
    ...
    ChessMove[] MoveOrder(boolean is_maximizer){
        
        Piece[] pieces= board.getAllPieces();
        int piece_len = 32;//Piece arrangement in the piece array may be zigzag because of FEN                
        
        int rated_value;
        ChessMove[] moves=new ChessMove[0];
        for(int i=0; i<piece_len; i++){//Piece arrangement in the piece array may be zigzag because of FEN            
            ...
            for(int square=0; square<valid_square.length; square++){
                ...
                rated_value=getPruneHelperValue(pieces[i], valid_square[square],
                                        enpassant_capture_square,promotion_piece_rating,is_maximizer);
                ...
                //let the killer moves come first for better prunning
    ...
    int getPruneHelperValue(Piece piece, int to_square,
                            int enpassant_capture_square,int promotion_piece_rating,boolean is_maximizer){
        
        int total_cost=0;
        int attack_cost=0;
        int threat_cost=0;
        
        if(enpassant_capture_square!=Constants.NOTHING){// is en passant capture
    ...
    private int search(boolean is_maximizer,
        ...
        if(n_depth < max_depth){
            ...
            ChessMove[] legal_moves = MoveOrder(is_maximizer);
                ...
                if(is_maximizer){            
                        ...
                        if(n_depth==0){                                                      
                            if(this.legalMoves==null)
                               this.legalMoves = legal_moves;//used for chess commentary by comparing moves value                           
