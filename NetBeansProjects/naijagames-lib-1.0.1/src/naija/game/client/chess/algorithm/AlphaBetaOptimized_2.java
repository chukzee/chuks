 ...
 */
public class AlphaBetaOptimized_2 {
    ...
    boolean is_exact = true;
    boolean is_lower_bound = true;
    boolean is_upper_bound = true;
    
    int move_length =40;
    
    public AlphaBetaOptimized_2() {
    }
   ...
   //To be remove - used for testing
    public static void main (String args[]) {
        AlphaBetaOptimized_2 ab=new AlphaBetaOptimized_2();

        long time=System.nanoTime();        
        ab.board=new Board(true);
        ab.engine_board_analyzer = new EngineBoardAnalyzer(ab.board);
        long value=ab.search(true, Long.MIN_VALUE, Long.MAX_VALUE, -1, 7 , -1);
        ...
        System.out.println("move_max_length "+ab.move_max_length );        
    ...
    public int search(boolean is_maximizer,
        ...
        if(ttHashTable.isEntryFound()){
            ...
            int tt_value=ttHashTable.getEval();
            if(tt_dept <= n_depth){//using <= comparision since it is normal (non-conventional) depth count.    
                if(ttHashTable.isExact()){
                    tt_prune_count++;//come back                 
                    return tt_value;// since we already know the value                  
                }else if(ttHashTable.isLowerBound() && tt_value > alpha){
                    alpha = tt_value;
                }else if(ttHashTable.isUpperBound() && tt_value < beta){
                    beta = tt_value;
                }
        ...
        for (int i=0; i<moves.length && moves[i]!=0 ;i++)
        {
            ...
            board.UndoMove(moves[i], caputre_id);//chuks
        ...
        if(eval_move_index != -1){
            if(value < alpha){
                ttHashTable.setEntry(hash_key, collision_filter, n_depth, moves[eval_move_index], 
                                     value, false, is_lower_bound, false);
            }else if(value > beta){
                ttHashTable.setEntry(hash_key, collision_filter, n_depth, moves[eval_move_index], 
                                     value, false, false, is_upper_bound);                
            }else{
                ttHashTable.setEntry(hash_key, collision_filter, n_depth, moves[eval_move_index], 
                                     value, is_exact, false, false);                
            }
        }                                    
