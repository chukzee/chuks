...
class ServerObjectImpl<K, V> extends AbstractServerObject {
    ...
    @Override
    public String[] getSessionsIDs(String attr_name, Object attr_value) {
    ...
    private void validatePdf() throws HttpServerException {
        if (html_buff != null) {
            if (html_buff.length() > 0) {
                throw new HttpServerException("data corruption - html mixed with pdf");
            }
        }
        if (pdf_buff != null) {
            if (pdf_buff.length > 0) {
                throw new HttpServerException("data corruption - pdf mixed with pdf");
            }
        }
    }
    ...
    public void pdf(byte[] pdf_data) {
        try {
            ...
            exception = new HttpServerException(ex.getMessage());
        }
    ...
    public void putCache(Serializable key, Serializable value) throws IOException {
        ...
        if (defaultEntryAttr.isDistributed()) {
            RemoteCachePacket rmtEntry = new RemoteCachePacket(key, value,
                    HttpServerImpl.getStrCacheSockAddr(), defaultEntryAttr);
    ...
    public void putCache(Serializable key, Serializable value, IEntryAttributes attr) throws IOException {
        ...
        if (attr.isDistributed()) {
            RemoteCachePacket rmtEntry = new RemoteCachePacket(key, value,
                    HttpServerImpl.getStrCacheSockAddr(), attr);
            rmtEntry.setCacheRegionName(jcsDefaultCache.getCacheName());
            TCPCacheTransport.enqueueCache(rmtEntry);
        }
    ...
    public void putRgnCache(String region_name, Serializable key, Serializable value) throws IOException {
        ...
        if (cattr != null && cattr.getIsRemote()) {
            RemoteCachePacket rmtEntry = new RemoteCachePacket(key, value,
                    HttpServerImpl.getStrCacheSockAddr(), null);
    ...
    @Override
    public void createCacheRegion(ICacheProperties config) throws ServerConfigException {
        if (config.getCacheRegionName() == null) {
            throw new ServerConfigException("cache region name cannot be null");
        }
        if (config.getCacheRegionName().isEmpty()) {
            throw new ServerConfigException("cache region name cannot be empty");
        }
        if (config.getCacheRegionName().equals(ServerConfig.INTERNAL_SEVER_SESSION_REGION_NAME)) {
            throw new ServerConfigException(config.getCacheRegionName() + " is an internal cache region name and cannot be used by client!");
        }
    ...
    private void removeCache(CompositeCache cache, Serializable key) {
        ...
        RemoteCachePacket rmtEntry = new RemoteCachePacket(key,
                HttpServerImpl.getStrCacheSockAddr());
