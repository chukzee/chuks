...
import chuks.server.ServerConfigException;
...
public class HttpServerImpl implements HttpServer {
    ...
    static {
        ...
        try {
            ...
            Logger.getLogger(HttpServerImpl.class.getName()).log(Level.SEVERE, null, e);
        }
        System.out.println("INFO: Done loading JDBC driver.");

    }

    private HttpServerImpl(ServerBuilder builder) {
        host = builder.host;
        port = builder.port;
    ...
    public boolean start() {
        try {
            if (isStarted) {
                throw new HttpServerException("another instance of this server is already running!");
            }
            ...
            isStarted = true;
            return isStarted;
        } catch (ServerConfigException | HttpServerException | IOException ex) {
    ...
    @Override
    public boolean stop(int wait_duration) {
        finalizeOperation();
        
        ServerHandler.stop();
        EventPacketHandler.stop();
        WebAppManager.stop();
        for(int i=0; i<wait_duration; i++){
            if(ServerHandler.isServerHandlerTerminated
                    &&WebAppManager.isStop()
                    &&EventPacketHandler.isStop()){
                break;
            }
            try {
    ...
    private void checkConfig() throws ServerConfigException {

        if (classPath == null || classPath.isEmpty()) {
            throw new ServerConfigException("invalid class path - null or empty");
        }

        if (root_dir == null || root_dir.isEmpty()) {
            throw new ServerConfigException("invalid root dir - null or empty");
        }

        if (new File(classPath).compareTo(new File(root_dir)) <= 0) {
            throw new ServerConfigException("invalid class path - class path must be sub directory of web root");
        }
        ...
        if (libraryPath != null) {
            ...
            if (new File(libraryPath).compareTo(new File(classPath)) < 0) {
                throw new ServerConfigException("invalid library path - library path must be same or sub directory of classpath");
            }
        }
    ...
    private void validateCacheSocketAddresses() throws ServerConfigException {

        try {
            cache_host_ip = InetAddress.getByName(host).getHostAddress();
        } catch (UnknownHostException ex) {
            throw new ServerConfigException(ex.getMessage(), ex);
        }

        if (cache_port == port) {
            throw new ServerConfigException("invalid cache port number (" + cache_port + ") - must not be same as that used by the server to listen for request");
        }
        try {
            ...
            try {
                if (InetAddress.getByName(rmt_host).equals(InetAddress.getByName(host))) {
                    if (Integer.parseInt(rmt_port) == cache_port) {
                        ...
                        Logger.getLogger(HttpServerImpl.class.getName()).log(Level.WARNING, warning);
                    }
                } else {
                    if (Integer.parseInt(rmt_port) != cache_port) {
                        throw new ServerConfigException("invalid remote cache port number detected - remote cache port number must be same as cache port used by this server expect in the case where the specified remote hostname is same as that of this server.");
                    }
                }
            } catch (UnknownHostException ex) {
                throw new ServerConfigException(ex.getMessage(), ex);
    ...
    public static class ServerBuilder {
        ...
        public HttpServer build() throws ServerConfigException {
            ...
            if (configFileToSave != null && !configFileToSave.trim().isEmpty()) {
                ...
                if (index == -1) {
                    ...
                    throw new ServerConfigException("invalid configuration file type - expected property file e.g someName.properties");
                }
        ...
        public HttpServer buildUsingConfig(Properties properties) {
            ...
            /*String type = ServerUtil.getFileType(config_file);
             ...
             throw new ServerConfigException("invalid configuration file type - expected property file");
             }*/
        ...
        public HttpServer buildUsingConfig(String config_file) throws ServerConfigException, IOException {
