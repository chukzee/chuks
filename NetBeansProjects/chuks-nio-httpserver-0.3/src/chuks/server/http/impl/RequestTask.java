...
import static chuks.server.http.impl.HttpServerImpl.getLoadBalanceStrategy;
...
class RequestTask implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            Logger.getLogger(HttpServerImpl.class.getName()).log(Level.SEVERE, null, ex);
        } catch (HttpContentLengthException | HttpServerException ex) {
            Logger.getLogger(RequestTask.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void handleRequest(byte[] arr, int offset, int size) throws UnsupportedEncodingException, IOException, HttpServerException {
    ...
    void analyzeHttpRecv(byte[] arr) throws HttpContentLengthException, UnsupportedEncodingException, IOException, HttpServerException {
        ...
        if (isAllHttpHeadersEnd) {
            ...
            if (size_recv_after_header <= content_len_remain) {
                handleRequest(arr, 0, current_read_size);
                return;
            }
        ...
        request.setRequest(new String(http_headers_bytes, 0, http_headers_bytes.length));                
        ...
        if (http_headers_bytes.length > request_length) {
            ...
            handleRequest(http_headers_bytes, headersLength, http_headers_bytes.length);
        }
