...
var tradesPending = function (sObj, market) {

    this.spotFxPending = function (user, input, res, recursion_count) {
        var user_exchange_id = user.exchangeId;
        var total = 0;
        var table = 'pending_positions_spotfx';
                ...
                .then(function (rows) {
                    ...
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        var row = {
                            ...
                            time: rows[i].TIME,
                            pending_order_expiry: rows[i].PENDING_ORDER_EXPIRY,
                            ...
                            pending_order_price: rows[i].PENDING_ORDER_PRICE,
                            open: rows[i].OPEN,
    ...
    this.optionsPending = function (user, input, res, recursion_count) {

        var user_exchange_id = user.exchangeId;
        var total = 0;
        var table = 'pending_positions_options';
        var condition = sObj.db.raw("(BINARY SELLER_ID=? AND DEL_BY_SELLER_ID is NULL)"
                + " OR "
                + "(BINARY BUYER_ID=? AND DEL_BY_BUYER_ID is NULL)",
                [user_exchange_id, user_exchange_id]);
