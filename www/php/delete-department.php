...
function deleteDepartment($app) {
    ...
    try {
        $stmt = $app->sqlDelete("add_department", "SN=? AND ENTRY_USER_ID=?", array($sn, $app->userSession->getSessionUsername()));

        if ($stmt->rowCount() > 0) {
            $app->sendSuccessJSON("Department deleted successfully!", null);
        } else {
