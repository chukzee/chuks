...
deleteServiceType($app);

function deleteServiceType($app) {
    ...
    $stmt = $app->sqlDelete("add_service_type", "SN=? AND ENTRY_USER_ID=?", array($sn, $app->userSession->getSessionUsername()));

    if ($stmt->rowCount() > 0) {
        $app->sendSuccessJSON("Service type deleted successfully!", null);
    } else {
