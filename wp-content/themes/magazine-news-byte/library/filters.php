...
function hoot_archive_title_filter( $title ) {
	...
	elseif ( is_category() )
		$title = str_replace( __( 'Category: ', 'magazine-news-byte' ), '', $title );

	elseif ( is_tag() )
		$title = str_replace( __( 'Tag: ', 'magazine-news-byte' ), '', $title );

	elseif ( is_tax() && !is_tax( 'post_format' ) )
		$title = single_term_title( '', false );

	elseif ( is_author() )
		$title = str_replace( __( 'Author: ', 'magazine-news-byte' ), '', $title );
	...
	elseif ( is_post_type_archive() )
		$title = str_replace( __( 'Archives: ', 'magazine-news-byte' ), '', $title );
	...
	if ( ( ( $page = get_query_var( 'paged' ) ) || ( $page = get_query_var( 'page' ) ) ) && $page > 1 ) {
		...
		$suffix = ' <span class="loop-title-suffix">' . sprintf( __( '(Page %s)', 'magazine-news-byte' ), $paged ) . '</span>';
...
function hoot_readmore_quicktag_replace( $text, $raw_excerpt ) {
	...
	return str_replace( '<!--hoot-read-more-quicktag-->', $read_more, $text );
}
endif;
