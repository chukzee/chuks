...
function magnb_add_appearance_subpage() {
	...
	add_theme_page(
		__( 'Magazine News Byte', 'magazine-news-byte' ), // Page Title
		__( 'About Theme Options', 'magazine-news-byte' ), // Menu Title
		'edit_theme_options', // capability
		'magazine-news-byte-welcome', // menu-slug
...
function magnb_welcome_notice(){
	if ( isset( $_GET['page'] ) && $_GET['page'] == 'magazine-news-byte-welcome' )
		return;
	if ( ! get_option( 'magazine-news-byte-dismiss-welcome' ) ) {
...
function magnb_add_welcome_notice() {
	...
	<div id="hoot-welcome-msg" class="notice notice-success is-dismissible">
		<p><?php
			...
			printf( esc_html__( 'Thank you for choosing Magazine NewsByte! To get started and fully take advantage of our theme, please make sure you visit the welcome page for the %1$sQuick Start Guide%2$s.', 'magazine-news-byte' ), '<a href="' . esc_url( admin_url( 'themes.php?page=magazine-news-byte-welcome&tab=qstart' ) ) . '">', '</a>' );
			?></p>
		<p><?php
			/* Translators: 1 is the link start markup, 2 is link markup end */
			printf( esc_html__( '%1$sGet started with Magazine NewsByte%2$s', 'magazine-news-byte' ), '<a class="button-secondary" href="' . esc_url( admin_url( 'themes.php?page=magazine-news-byte-welcome&tab=qstart' ) ) . '">', '</a>' );
			?></p>
	</div>
	<?php
}
...
function magnb_dismiss_welcome_notice() {
	...
	update_option( 'magazine-news-byte-dismiss-welcome', 1 ); // echo 'dismiss test';
	wp_die();
}
...
function magnb_admin_enqueue_about_styles( $hook ) {
	if ( $hook == 'appearance_page_magazine-news-byte-welcome' )
...
function magnb_appearance_subpage() {
	...
	<div class="wrap">
		...
		<h1 class="hoot-about-title"><?php
			...
			printf( esc_html__( 'About %1$s %2$s', 'magazine-news-byte' ), hoot_data( 'template_name' ), hoot_data( 'template_version' ) );
			?></h1>
		...
		<div id="hoot-about-sub" class="hoot-about-sub">
			...
			<div class="hoot-about-text">
				<p class="hoot-about-intro"><?php esc_html_e( 'Magazine NewsByte is a multipurpose highly flexible WordPress theme built on a SEO friendly framework with fast loading speed. It comes with multiple Theme Customizer options, various blog layouts, WooCommerce support etc among many other features.', 'magazine-news-byte' ) ?></p>
				<p class="hoot-about-textlinks">
					<a class="button button-primary" href="https://wphoot.com/themes/magazine-news-byte/" target="_blank"><span class="dashicons dashicons-dashboard"></span> <?php esc_html_e( 'View Premium', 'magazine-news-byte' ) ?></a>
					<a class="button" href="https://demo.wphoot.com/magazine-news-byte/" target="_blank"><span class="dashicons dashicons-welcome-view-site"></span> <?php esc_html_e( 'View Demo', 'magazine-news-byte' ) ?></a>
					<a class="button" href="https://wphoot.com/support/magazine-news-byte/" target="_blank"><span class="dashicons dashicons-editor-aligncenter"></span> <?php esc_html_e( 'Documentation', 'magazine-news-byte' ) ?></a>
					...
					<a class="button" href="https://wordpress.org/support/theme/magazine-news-byte/reviews/?filter=5#new-post" target="_blank"><span class="dashicons dashicons-thumbs-up"></span> <?php esc_html_e( 'Rate Us', 'magazine-news-byte' ) ?></a>
				</p>
				<?php do_action( 'magnb_theme_after_about_textlinks', 'magazine-news-byte' ); ?>
				<?php /*
				<div class="hoot-about-notice2">
					<h3><?php esc_html_e( '1 Click Demo Installation', 'magazine-news-byte' ) ?></h3>
					<p><?php
						/* Translators: The %s are placeholders for HTML, so the order can't be changed. *//*
						printf( esc_html__( 'Use the OCDI plugin to install demo content with a single click to make your site look exactly like the %1$sDemo Site%2$s.%3$sNew users often find it easier to edit existing content rather than starting from scratch.', 'magazine-news-byte' ), '<a href="https://demo.wphoot.com/magazine-news-byte/" target="_blank">', '</a>', '<br />' );
					?></p>
					<p class="hoot-about-textlinks">
						<a class="button" href="https://wphoot.com/support/magazine-news-byte/#docs-section-demo-content-free" target="_blank"><span class="dashicons dashicons-art"></span> <?php esc_html_e( 'Install Demo Content', 'magazine-news-byte' ) ?></a>
					</p>
				</div>
				*/ ?>
			</div>
		...
		<div class="hoot-abouttabs">
			...
			<h2 id="nav-tabs" class="nav-tab-wrapper">
				...
				<?php do_action( 'magnb_theme_after_nav_tab_upsell', 'magazine-news-byte', $activetab ); ?>
			...
			<div id="hoot-upsell" class="hoot-upsell hoot-tab <?php if ( $activetab == 'upsell' ) echo 'hoot-tab-active'; ?>">
				<h2 class="centered allcaps"><?php
					/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
					printf( esc_html__( 'Upgrade to %1$sMagazine NewsByte %2$sPremium%3$s%4$s', 'magazine-news-byte' ), '<span>', '<strong>', '</strong>', '</span>' );
					?></h2>
				<p class="hoot-tab-intro centered"><?php
					/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
					printf( esc_html__( 'If you have enjoyed using Magazine NewsByte, you are going to love %1$sMagazine NewsByte Premium%2$s.%3$sIt is a robust upgrade to Magazine NewsByte that gives you many useful features.', 'magazine-news-byte' ), '<strong>', '</strong>', '<br />' );
					?></p>
				<p class="hoot-tab-cta centered">
					<a class="button button-secondary secondary-cta" href="https://demo.wphoot.com/magazine-news-byte/" target="_blank"><span class="dashicons dashicons-welcome-view-site"></span> <?php esc_html_e( 'View Demo Site', 'magazine-news-byte' ) ?></a>
					<a class="button button-primary primary-cta" href="https://wphoot.com/themes/magazine-news-byte/" target="_blank"><span class="dashicons dashicons-dashboard"></span> <?php esc_html_e( 'Buy Magazine NewsByte Premium', 'magazine-news-byte' ) ?></a>
				</p>
			...
			<?php do_action( 'magnb_theme_after_hoot_upsell', 'magazine-news-byte', $activetab ); ?>
			...
			<div id="hoot-qstart" class="hoot-qstart hoot-tab <?php if ( $activetab == 'qstart' ) echo 'hoot-tab-active'; ?>">
				...
				<p class="hoot-tab-intro centered"><?php
					...
					?></p>
				<p class="hoot-tab-cta centered">
					<a class="button button-primary primary-cta" href="https://wphoot.com/support/magazine-news-byte/" target="_blank"><span class="dashicons dashicons-editor-aligncenter"></span> <?php esc_html_e( 'View Full Documentation', 'magazine-news-byte' ) ?></a>
...
function magnb_upstrings( $string ) {
	...
	$features[] = array(
		...
		'desc' => esc_html__( 'Different looks and styles. Choose from an extensive range of customization features in Magazine NewsByte Premium to setup your own unique front page. Give youe site the personality it deserves.', 'magazine-news-byte' ),
	...
	$features[] = array(
		...
		'desc' => esc_html__( 'Magazine NewsByte Premium lets you select custom colors and backgrounds for different sections of your site like header, footer, logo background, menu dropdown, content area, page title area etc.', 'magazine-news-byte' ),
	...
	$features[] = array(
		...
		/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
		'desc' => sprintf( esc_html__( 'Magazine NewsByte Premium allows you to create unlimited sliders with as many slides as you need using the awesome HootKit plugin.%1$s%2$sAdd as Shortcodes%3$sYou can use these sliders on your Frontpage, or add them anywhere using shortcodes - like in your Posts, Sidebars or Footer.', 'magazine-news-byte' ), '<hr>', '<h4>', '</h4>' ),
	...
	$features[] = array(
		...
		'desc' => esc_html__( 'Magazine NewsByte Premium gives you the option to display your post archives in different layouts including a mosaic type layout similar to pinterest.', 'magazine-news-byte' ),
	...
	$features[] = array(
		...
		/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
		'desc' => sprintf( esc_html__( 'Additional Hootkit Custom widgets crafted and designed specifically for Magazine NewsByte Premium Theme to give you the flexibility of adding stylized content: %1$s Buttons, Carousel Sliders, Carousel Posts Slider, Contact Info, Icon Lists, Notices, Number Blocks, Tabs, Toggles and Vcards among others.', 'magazine-news-byte' ), '<hr>' ),
	...
	$features[] = array(
		...
		'desc' => esc_html__( 'Magazine NewsByte Premium comes with many additional premium background patterns. You can always upload your own background image/pattern to match your site design.', 'magazine-news-byte' ),
	...
	$features[] = array(
		...
		/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
		'desc' => sprintf( esc_html__( 'Automatically open image links on your site with the integrates lightbox in Magazine NewsByte Premium.%1$sAutomatically convert standard WordPress galleries to beautiful lightbox gallery slider.', 'magazine-news-byte' ), '<hr>' ),
	...
	$features[] = array(
		...
		'desc' => esc_html__( 'CSS is passe! Developers love the modularity and ease of using LESS, which is why Magazine NewsByte Premium comes with properly organized LESS files for the main stylesheet.', 'magazine-news-byte' ),
	...
	$features[] = array(
		...
		'blocks' => array(
			...
			array(
				...
				'desc' => esc_html__( 'You will help support the continued development of Magazine NewsByte - ensuring it works with future versions of WordPress for years to come.', 'magazine-news-byte' ),
	...
	$features[] = array(
		...
		/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
		'desc' => sprintf( esc_html__( 'Need help setting up Magazine NewsByte? Upgrading to Magazine NewsByte Premium gives you prioritized support. We have a growing support team ready to help you with your questions.%1$sNeed small modifications? If you are not a developer yourself, you can count on our support staff to help you with CSS snippets to get the look you are after. Best of all, your changes will persist across updates.', 'magazine-news-byte' ), '<hr>' ),
	...
	$quickstart[] = array(
		...
		'desc' => sprintf( esc_html__( '%14$sIt is recommended to install demo content only on a fresh new site%15$s
			...
			%2$s', 'magazine-news-byte' ), '<ol>', '</ol>', '<li>', '</li>', '<strong>', '</strong>', '<em>', '</em>', '<hr>',
										'<a href="https://demo.wphoot.com/magazine-news-byte/" target="_blank">',
										'<a href="https://wphoot.com/support/magazine-news-byte/#docs-section-demo-content-free" target="_blank">',
