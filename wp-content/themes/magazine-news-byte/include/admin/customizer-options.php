...
function magnb_customizer_options() {
	...
	$lcontent .= '<a class="hoot-cust-link" href="' .
				 ...
				 '</span></a>';
	$lcontent .= '<a class="hoot-cust-link" href="' .
				 'https://wordpress.org/support/theme/magazine-news-byte/reviews/?filter=5#new-post' .
				 '" target="_blank"><span class="hoot-cust-link-head">' .
	...
	$settings['post_featured_image'] = array(
		...
		'choices'     => array(
			...
			'header'  => esc_html__( 'Full width in header (Parallax Effect)', 'magazine-news-byte'),
	...
	$settings['contact-form'] = array(
		...
		'section'     => $section,
		'type'        => 'content',
		'priority'    => '1345', // Non static options must have a priority
		/* Translators: 1 is the link start markup, 2 is link markup end */
	...
	if ( current_theme_supports( 'woocommerce' ) ) :
		...
		$sections[ $section ] = array(
			'title'       => esc_html__( 'WooCommerce (NewsByte)', 'magazine-news-byte' ),
		...
		$settings['wooshop_products'] = array(
			'label'       => esc_html__( 'Total Products per page', 'magazine-news-byte' ),
			'section'     => $section,
			'type'        => 'select',
			'priority'    => '1365', // Non static options must have a priority
			'choices'     => $wooproducts,
			'default'     => '12',
			'description' => esc_html__( 'Total number of products to show on the Shop page', 'magazine-news-byte' ),
		);

		$settings['wooshop_product_columns'] = array(
			'label'       => esc_html__( 'Product Columns', 'magazine-news-byte' ),
			'section'     => $section,
			'type'        => 'select',
			'priority'    => '1365', // Non static options must have a priority
			'choices'     => array(
			...
			'default'     => '3',
			'description' => esc_html__( 'Number of products to show in 1 row on the Shop page', 'magazine-news-byte' ),
		);
