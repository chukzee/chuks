var str =`   Format L  Mode  Description
--------------------------------------------------------------------------------
...
CLIPBOARD S  rw-  Windows System Clipboard
    ...
    CMYKA S  rw+  Raw cyan, magenta, yellow, black, and opacity samples
      ...
      EMF S  r--  Windows WIN32 API rendered Enhanced Meta File
...
IPTCWTEXT P  rw-  IPTC Newsphoto text format
     ...
     JBIG P  rw+  Joint Bi-level Image experts Group interchange format (1.6)
      JNG S  rw-  JPEG Network Graphics (libpng 1.6.24, zlib 1.2.8)
    ...
    MATTE S  -w+  MATTE raw opacity format
     ...
     MIFF P  rw+  Magick Image File Format
      MNG S  rw+  Multiple-image Network Graphics (libpng 1.6.24, zlib 1.2.8)
   ...
   PLASMA S  r--  Plasma fractal image
      PNG P  rw-  Portable Network Graphics (libpng 1.6.24, zlib 1.2.8)
            See http://www.libpng.org/ for information on PNG..
    PNG00 P  rw-  PNG that inherits type and depth from original (libpng 1.6.24, zlib 1.2.8)
    PNG24 P  rw-  24-bit RGB PNG, opaque only (libpng 1.6.24, zlib 1.2.8)
    PNG32 P  rw-  32-bit RGBA PNG, semitransparency OK (libpng 1.6.24, zlib 1.2.8)
    PNG48 P  rw-  opaque or binary transparent 48-bit RGB (libpng 1.6.24, zlib 1.2.8)
    PNG64 P  rw-  opaque or transparent 64-bit RGBA (libpng 1.6.24, zlib 1.2.8)
     PNG8 P  rw-  8-bit indexed PNG, binary transparency only (libpng 1.6.24, zlib 1.2.8)
  ...
  STEGANO S  r--  Steganographic image
      ...
      SVG S  r--  Scalable Vector Graphics (XML 2.9.4)
     SVGZ S  r--  Scalable Vector Graphics (ZIP compressed) (XML 2.9.4)
     TEXT S  rw+  ASCII Text
      TGA S  rw+  Truevision Targa image
     TIFF P  rw+  Tagged Image File Format (LIBTIFF, Version 4.0.9)
    ...
    VICAR S  rw-  VICAR rasterfile format
     ...
     WEBP P  rw-  WebP Image Format (libwepb v0.5.1, ENCODER ABI 0x0209)
 ...
 WMFWIN32 S  r--  Windows WIN32 API rendered Meta File
      ...
      XWD U  ---  X Windows system window dump (color)
        Y S  rw+  Raw yellow samples
      YUV S  rw-  CCIR 601 4:1:1 or 4:2:2 (8-bit only)

 Meaning of 'L': P=Primary, S=Stable, U=Unstable
gm convert: Unable to register image format ("PSD: The specified procedure could not be found.
...
for(var i=0; i<arr.length; i++){
    a = arr[i].trim();
    var line = a.split(' ');
    if((line[1] === 'P' || line[1] === 'S' || line[1] === 'U')
            && (line[3] === 'rw+' || line[3] === 'rw-' || line[3] === '-w-' || line[3] === '-w+' || line[3] === 'r--' || line[3] === '---')){
        if(line[1]==='U'){//skip unstable ones labelled as U
            continue;
        }
        
        if(line[3]==='-w-'){//skip write only operations -w-
            //continue;
        }
        var frm = a.split(' ');
        console.log(frm[0]);
